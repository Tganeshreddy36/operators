...
total_extras = (no_balls +   # 1 run per no-ball
                 wides +      # 1 run per wide
                 byes +       # 1 run per bye
                 leg_byes +   # 1 run per leg-bye
                 (penalties * 5))  # 5 runs per penalty
...
40
...
# Read the total age of the three players
total_age = int(input())
...
# Read the rate of interest per annum
rate = float(input())
